...
for i, s in enumerate(seed_list):
    if i % 2 == 0 and i < len(seed_list) - 1:
        seed_range_list += (seed_list[i], seed_list[i] + seed_list[i+1])
...
for key, value in maps.items():
    print(key, value)
    for i, list_of_source_ranges in enumerate(seed_range_list):
        converted = False
        for destination_start, source_start, range_length in value:
            for srs, sre in list_of_source_ranges:
                # contained
                if
                if srs >= source_start and srs < source_start + range_length:
                    pass
                if

            if range(s, e +1) in range(source_start, source_start + range_length):
            ...
            if s >= source_start and s < source_start + range_length:
                ...
                seed_range_list[i] = destination_start + (s - source_start)
                break
        if not converted:
            print(f"Seed {s} is not in range of {key}")

print(min(seed_range_list))
