...
for sequence in data[0].split(','):
    summation += hash(sequence)
print(summation)

### Holiday ASCII String Helper Manual Arrangement Procedure
def show_boxes(boxes: list[list], focal_lengths: dict):
    for i in range(len(boxes)):
        if boxes[i] == []:
            continue
        concat = ""
        for b in boxes[i]:
            concat += f" [{b} {focal_lengths[b]}]"
        print(f"Box: {i}{concat}")


boxes: list[list[str]] = [[]] * 256
...
for sequence in data[0].split(','):
    ...
    if sequence.find('=') >= 0:
        ...
        label = a
        box = hash(a)
        print(f"Working on box {box}")
        focal_length = int(b)
        focal_lengths[label] = focal_length
        if boxes[box].count(label) > 0:
            focal_lengths[label] = focal_length
        else:
            print(f"Adding {label} to box {box}")
            boxes[box] = [*boxes[box], label]
...
show_boxes(boxes, focal_lengths)

focusing_power = 0
for box_number, box in enumerate(boxes):
    for slot, label in enumerate(box):
        focusing_power += (box_number + 1) * (slot + 1) * focal_lengths[label]
